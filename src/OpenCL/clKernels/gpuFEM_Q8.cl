...
void getdShapeMat(uint       gp,
                  ...
                  __local    fem_float dShapeMatrix[2][8]);
                  void       local_getJacobianMatrix(fem_float Jacobian[2][2],
                  __local    fem_float coordsElem[2][8],
                  __local    fem_float dShapeFunc[2][8],
                  __local    fem_float aux[8]);
// Returns derived shape function matrix for given gauss points
...
void      local_matMult_2_2_8( uint lidx, __local fem_float matrixC[2][8], fem_float matrixA[2][2], __local fem_float matrixB[2][8] );
void      local_matMult_3_3_16( uint lidx, __local fem_float matrixC[3][16], fem_float matrixA[3][3], __local fem_float matrixB[3][16] );
void      local_matAddMultTranspScal_16_3_16( uint lidx, __local fem_float matrixC[16][16], __local fem_float matrixA[3][16],
                                              __local fem_float matrixB[3][16], fem_float scal );
void      local_buildBMatrix( uint lidx, __local fem_float matrixB[3][16], __local fem_float dNdcart[2][8]  );
void      writeToGlobalKaux( uint lidx, uint elem, __local fem_float k_local[16][16], __global float* global_Kaux);
void      printMatrix(__local float mat[16][16]);
...
void getStiffnessQ8(__private  fem_float  E,
                    ...
                    __global   fem_float* global_Kaux)//output
{
  ...
  local fem_float elemCoords[2][8];
  local fem_float dShapeMat[2][8];
  local fem_float dNdCart[2][8];
  local fem_float B[3][16];
  local fem_float CxB[3][16];
  local fem_float k_local[16][16];
  ...
  elemCoords[0][lidx] = nodeCoords[(elemConnect[8*elem+lidx]-1)*2  ];
  elemCoords[1][lidx] = nodeCoords[(elemConnect[8*elem+lidx]-1)*2+1];
  ...
  for( uint gp=0; gp<(gpts*gpts); gp++)
  {
    ...
    local_matMult_2_2_8( lidx, dNdCart, J_inv, dShapeMat);
    local_buildBMatrix( lidx, B, dNdCart );
    local_matMult_3_3_16( lidx, CxB, C, B );
    local_matAddMultTranspScal_16_3_16( lidx, k_local, B, CxB, (detJ*W_gaussweights[gp]) );
  }
...
////////////////////////////////////////////////////////////////////////////////
void getdShapeMat(uint gp, __constant fem_float* rst, __local fem_float dShapeMatrix[2][8]) {
  ...
  // Shape func for corner nodes:   Ni = 1/8(1+r*ri)(1+s*si)(1+t*ti)
    if( get_local_id(0) == 0 )
    {
      ...
      dShapeMatrix[0][0] = -0.25f*((2.0f*r + s)*(s - 1));
      dShapeMatrix[0][1] = r*(s - 1);
      dShapeMatrix[0][2] = -0.25f*((2*r - s)*(s - 1));
      dShapeMatrix[0][3] = 0.5f * (1 - (s*s));
      dShapeMatrix[0][4] = 0.25f*((2*r + s)*(s + 1));
      dShapeMatrix[0][5] = -r*(s + 1);
      dShapeMatrix[0][6] = 0.25f*((2*r - s)*(s + 1));
      dShapeMatrix[0][7] = 0.5f * ((s*s) - 1);
      //// dShapeMatrix[1][i] = d/ds[Ni]
      dShapeMatrix[1][0] = -0.25f*((r + 2*s)*(r - 1));
      dShapeMatrix[1][1] = 0.5f * ((r*r) - 1);
      dShapeMatrix[1][2] = -0.25f*((r - 2*s)*(r + 1));
      dShapeMatrix[1][3] = -s*(r + 1);
      dShapeMatrix[1][4] = 0.25f*((r + 2*s)*(r + 1));
      dShapeMatrix[1][5] = 0.5f * (1 - (r*r));
      dShapeMatrix[1][6] = 0.25f*((r - 2*s)*(r - 1));
...
////////////////////////////////////////////////////////////////////////////////
void local_getJacobianMatrix( fem_float Jacobian[2][2], __local fem_float coordsElem[2][8], __local fem_float dShapeFunc[2][8], __local fem_float* aux )
{
...
fem_float localDotProd( __local fem_float* v1, __local fem_float* v2, __local fem_float* aux )
{
  ...
  fem_float sum1 = aux[0]+aux[1];
  fem_float sum2 = aux[2]+aux[3];
  fem_float sum3 = aux[4]+aux[5];
  fem_float sum4 = aux[6]+aux[7];
...
// Computes Matrix Multipication for 2x2 * 2x8 matrices
////////////////////////////////////////////////////////////////////////////////
void local_matMult_2_2_8( uint lidx, __local fem_float matrixC[2][8], fem_float matrixA[2][2], __local fem_float matrixB[2][8] )
{
  barrier(CLK_LOCAL_MEM_FENCE);
...
// Computes Matrix Multipication for 3x3 * 3x16 matrices
////////////////////////////////////////////////////////////////////////////////
void local_matMult_3_3_16( uint lidx, __local fem_float matrixC[3][16], fem_float matrixA[3][3], __local fem_float matrixB[3][16] )
{
  barrier(CLK_LOCAL_MEM_FENCE);
...
void writeToGlobalKaux( uint lidx, uint elem, __local fem_float k_local[16][16], __global float* global_Kaux)
{
  uint elemStride = elem*16;
  uint rowStride  = 16*get_num_groups(0);
